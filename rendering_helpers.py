...
def get_rays(height: int, width: int, focal_length, c2w: torch.Tensor,negative_z=True) -> Tuple[torch.Tensor, torch.Tensor]:
    r"""
    ...
    if negative_z==True:
        x_vals = (i - width * 0.5) / fx
        y_vals = -(j - height * 0.5) / fy
        z_vals = -torch.ones_like(i)
    else:
        y_vals = (j - height * 0.5) / fy
        z_vals = +torch.ones_like(i)
        #print("Positive Z Used")
...
def get_rays_DEPRRECATED(
    height: int, width: int, focal_length: float, c2w: torch.Tensor,negative_z=True
) -> Tuple[torch.Tensor, torch.Tensor]:
    r"""
    Find origin and direction of rays through every pixel and camera origin.
    """
    ...
    i, j = i.transpose(-1, -2), j.transpose(-1, -2)
    if negative_z==True:
        y_vals = -(j - height * 0.5) / focal_length
        z_vals = -torch.ones_like(i)
    else:
        y_vals = (j - height * 0.5) / focal_length
        z_vals = +torch.ones_like(i)
