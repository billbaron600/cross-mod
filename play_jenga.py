...
#Specify key paths
configuration_path = "play_jenga_initial_demos.json"
...
full_limit_to_correction_indices = [17]


#Success Seeds: [1,2,3,4,8,9,11,13,15,17,18]
#Fail seeds: [5,6,7,10,12,14,16,19,20]


shift_mean = np.array([0,0.0,0.0]) #slight shift to the mean
min_vals = np.array([-10.0,-10.0,0.761]) #min xyz values in the bounding box. Just dont have waypoitns outside this
max_vals = np.array([10.0,10.0,1.45])
cam_pose = [1.0,0.0,1.0]
open_gripper = 1.0
...
print_info = False
model_idx = 50000
model_path = "run_results/play_jenga/demos/RL_models/behavior_cloned_model"
#model_path = "run_results/play_jenga/demos/policy_models/model_6000"
buffer_path = None
...
if update_full_ivk_pickle is True:
    merge_full_trajectories(config, results_path,limit_to_correction_indices=full_limit_to_correction_indices)

if plot_result_from_ivk is True:
    #Plot the demo data
    plot_trajectory_grid(config,results_path,limit_to_correction_indices=full_limit_to_correction_indices)
...
if evaluate_policy_bool is True:
    ...
    model = SACQuatSafe.load(path_to_model,env=vec_env)
    model.policy.eval()
