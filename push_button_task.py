...
#Specify key paths
configuration_path = "push_button_config_initial_demos.json"
path_to_config_pickle = "run_results/push_button/demos/config_instance.pkl"
task = PushButton
results_path = "run_results/push_button/demos/"
...
full_limit_to_correction_indices = [10]
...
shift_mean = np.array([0,0,0.0]) #shift the mean up by 0.02 meters
#shift_mean = None
mean_shift_indices = []
...
distance_between_points=0.005 #set to none to use the mean
NUM_POINTS_MEAN = 90
NUM_POINTS_VAR = 5
NUM_SPEEDS = 1
NUM_TRAJECTORIES = 1
#FOR ALL SEEDS OTHER THAN 1:
...
#OTEHR ARGS
config.ray_tracing_params["n_views"] = [0,1]
config.ray_tracing_params["ray_dist_threshold"] = 0.05
config.ray_tracing_params["density_threshold"] = 0.25
config.ray_tracing_params["focal"] = -129.9
#config.ray_tracing_params["density_threshold"] = [0,1]
config.density_generation_params["n_images"] = 2

config.draw_traj_config["n_images"] = 2
config.ivk_generation_kwargs["open_gripper"] = 0.1
config.execute_trajectory_kwargs["offset_by"][4]=[0,0,0]
config.ray_tracing_params["n_samples"] = 150
config.ray_tracing_params["far"] = 4

#Timestep stuff
config.density_generation_params["time_length"] = 50
config.ray_tracing_params["n_times"] = 50
...
#Ray tracing args
"""
config.ray_tracing_params["n_weights"] = n_weights
config.ray_tracing_params["gamma"] = gamma


#IVK ARGS
config.execute_trajectory_kwargs["offset_by"][0]=[0,0,0]
config.execute_trajectory_kwargs["steps_to_interp_quaternion"] = 300
config.ivk_generation_kwargs["open_gripper"] = 0.0
config.ivk_generation_kwargs["record_video"]=record_ivk_video
...
for corr_idx in full_limit_to_correction_indices:
    ...
    for segment_idx in range(n_segments):
        ...
        if fit_NeRF_bool is True:
            ...
            fit_NeRF_model(config,limit_to_correction_indices=limit_to_correction_indices,mean_shift_indices=[],shift_mean=shift_mean,negative_z=negative_camera_z,segment_idx=segment_idx)
    ...
    if n_segments>0:
        combine_segments(config,limit_to_correction_indices=limit_to_correction_indices)
    ...
    if run_IVK_bool is True:
        generate_IVK_trajectories(config,task=task,limit_to_correction_indices=limit_to_correction_indices,trajectory_indices=trajectory_indices,discrete_gripper=False,use_gripper_action_file=False,use_mean=False,use_gripper_orientation_file=False)
        #Plot the demo data
