...
<p class="text">
It is therefore useful to distinguish between <strong>semantic task understanding</strong> and <strong>fine grained spatial localization</strong>. The first is the ability to map language and images to goals, constraints, and subgoals. The second is the ability to identify contact locations, trajectories, and free space with the precision required for reliable manipulation.
</p>
...
<p class="text">
On Where2Place, the reported accuracies are approximately 38.8 for Gemini 2.0 Pro Experimental, 45.0 for Gemini Robotics ER, 20.6 for GPT 4o, 16.2 for Claude 3.5 Sonnet, 45.0 for Molmo 7B D, and 63.8 for Molmo 72B. Even under a shared evaluation protocol, state of the art chat oriented VLMs remain far behind both a robotics fine tuned Gemini variant and the open weight Molmo models on this dense pointing task.
</p>

<p class="text">
The <a href="https://arxiv.org/abs/2409.17146" target="_blank" rel="noopener">Molmo and PixMo</a> work provides an important contrast. Molmo 7B D uses a 7 billion parameter backbone but already matches or exceeds much larger proprietary VLMs on several pointing benchmarks. The 72B version achieves the strongest reported Where2Place score. Both models are trained with the PixMo suite, which includes a large 2D pointing dataset collected specifically for precise localization. In other words, a comparatively small VLM that is explicitly trained for pointing can outperform much larger generalist models on spatial precision, and can remain competitive even with a robotics specialized model like Gemini Robotics ER.
</p>

<p class="text">
These results indicate that high level multimodal reasoning and fine grained spatial localization behave as at least partially independent capability axes. Scaling a generalist model improves pointing, but targeted pointing data and objectives provide much larger gains. We refer to the resulting performance difference between semantic reasoning and precise localization as the <strong>spatial precision gap</strong>. Later sections of this page and the paper will examine how modular architectures can narrow this gap without continually retraining a single monolithic model.
</p>
...
<p class="text">
The <strong>reasoning VLM</strong> takes images and language as input. It interprets the task, identifies relevant objects and constraints, and emits structured descriptors such as semantic keypoints, subgoal descriptions, or coarse trajectory sketches. Its parameters remain frozen across robotics applications so that general capabilities are preserved.
</p>
...
<p class="text">
A coupling module maintains shared context between the two components. Descriptors from the reasoning model are passed to the pointing model, which returns concrete coordinates. These coordinates are then fed back into the reasoning model or a downstream planner to update sketches, synthesize 3D waypoints, and generate control commands. Conceptually, this resembles a classical navigation stack where a global planner reasons over maps and goals, and a local controller uses sensor feedback to track trajectories and avoid obstacles. Here, the reasoning VLM plays the role of the global planner, while the pointing VLM behaves like a specialized estimator that turns symbolic references into metric contact points.
</p>

<p class="text">
This modular pattern offers several advantages. It preserves the general reasoning abilities of the large VLM, which can be reused across embodiments and tasks. It isolates the high variance spatial estimation problem into a smaller component that can be retrained or replaced without disturbing the rest of the system. It also exposes interpretable intermediate representations, such as named keypoints and trajectories, that can be logged, inspected, or constrained by classical control stacks and safety monitors.
</p>
...
<p class="text">
The separation between reasoning and pointing modules is also supported by evidence from visual neuroscience. When visual input reaches occipital cortex, it diverges into at least two major pathways. A ventral stream, projecting toward inferior temporal cortex, supports object recognition and semantic identification. A dorsal stream, projecting toward posterior parietal areas, supports spatial localization and visually guided action. Classic work by Goodale and Milner and subsequent reviews emphasize this division between perception for recognition and perception for action, with areas such as lateral occipital complex (LOC) and anterior intraparietal area (AIP) playing key roles.
</p>
...
<p class="text">
Modular reasoner pointer architectures take a similar approach. The reasoning VLM plays a ventral like role, maintaining an object and task centered description of the scene. The pointing VLM plays a dorsal like role, using that description to pick metrically accurate contact points in the image. The coupling between them is analogous to the recurrent interaction between temporal and parietal cortex. From both engineering and biological perspectives, separating these roles provides a principled way to narrow the spatial precision gap without repeatedly retraining a single monolithic model on increasingly specialized data.
</p>
